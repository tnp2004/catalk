...
type UserService interface {
	InsertUser(req *NewUserModel) (*UserEntity, error)
...
func (u *user) InsertUser(req *NewUserModel) (*UserEntity, error) {
	...
	if err := tx.QueryRowContext(ctx, query,
		...
		req.PictureURL,
		req.ProviderID,
	).Scan(&userEntity.ID, &userEntity.Email, &userEntity.Username, &userEntity.PictureURL,
		&userEntity.ProviderID, &userEntity.CreatedAt, &userEntity.UpdatedAt); err != nil {
		tx.Rollback()
